    ...
    class ChatCommands
    {
        ...
        public static bool Prefix(ChatController __instance)
        {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                switch (args[0])
                {
                    ...
                    case "/ban":
                        ...
                        if (subArgs == "show")
                        {
                            ...
                            foreach (var pc in PlayerControl.AllPlayerControls)
                            {
                                if (pc == null || pc.Data.Disconnected) continue;
                                string name = Main.devNames.ContainsKey(pc.PlayerId) ? Main.devNames[pc.PlayerId] : pc.GetRealName(true);
                                name = name.RemoveHtmlTags();//clean up /ban
                                sentttext += $"\n{name} : {pc.PlayerId}";
                            }
                            if (sentttext != "") HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, sentttext);
                        }
                    ...
                    case "/Rolestemplate":
                        canceled = true;
                        if (args.Length > 1) SendRoleTemplate(args[1]);
                        else HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, $"{GetString("ForExample")}:\n{args[0]} test");
                        break;
        ...
        public static void GetRolesInfo(string role)
        {
            var roleList = new Dictionary<CustomRoles, string>
            {
                ...
                //{ CustomRoles.Mayor, "my" },{ CustomRoles.Doctor, "doc" },{ CustomRoles.Transporter, "tr" },{ CustomRoles.Trapper, "tra" },{ CustomRoles.Demolitionist, "demo"},
                { CustomRoles.Bodyguard, "bd" },
                { CustomRoles.Oracle, "or" },
                { CustomRoles.Forecaster, "fc" },
                { CustomRoles.Medic, "me" },
                { CustomRoles.Paramedic, "par" },
               ...
               // { CustomRoles.Demolitionist, "demo"},
                ...
                { CustomRoles.Wizard, "wiz" },
                { CustomRoles.Reviver, "rev" },
                { CustomRoles.Chancer, "ch" },
                { CustomRoles.Sellout, "se" },
                { CustomRoles.Kamikaze, "kami"},
                //Neutral役職
                ...
                { CustomRoles.Lawyer, "la" },
                { CustomRoles.Hustler, "hu"},
                ...
                { CustomRoles.Veteran, "vet" },
                { CustomRoles.Escapist, "esc" },
                { CustomRoles.PUMPkinsPotion, "pkp" },
                //HAS
        ...
        public static void SendRoleTemplate(string str = "", byte playerId = 0xff, bool noErr = false)
        { 

            if (!File.Exists(Main.ROLES_FILE_PATH))
            {
                HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "No template file found.");
                File.WriteAllText(Main.ROLES_FILE_PATH, "test:This is template text.\\nLine breaks are also possible.\ntest:これは定型文です。\\n改行も可能です。");
                return;
            }
            using StreamReader rl = new(Main.ROLES_FILE_PATH, Encoding.GetEncoding("UTF-8"));
            string text1;
            string[] tmp1 = { };
            List<string> sendList1 = new();
            HashSet<string> tags1 = new();
            while ((text1 = rl.ReadLine()) != null)
            {
                tmp1 = text1.Split(":");
                if (tmp1.Length > 1 && tmp1[1] != "")
                {
                    tags1.Add(tmp1[0]);
                    if (tmp1[0] == str) sendList1.Add(tmp1.Skip(1).Join(delimiter: "").Replace("\\n", "\n"));
                }
            }
            if (sendList1.Count == 0 && !noErr)
            {
                if (playerId == 0xff)
                    HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, string.Format(GetString("Message.TemplateNotFoundHost"), str, tags1.Join(delimiter: ", ")));
                else Utils.SendMessage(string.Format(GetString("Message.TemplateNotFoundClient"), str), playerId);
            }
            else for (int i = 0; i < sendList1.Count; i++) Utils.SendMessage(sendList1[i], playerId);
        }
        ...
        public static void OnReceiveChat(PlayerControl player, string text)
        {
            ...
            switch (args[0])
            {
                ...
                case "/r":
                case "/Rolestemplate":
                    if (args.Length > 1) SendRoleTemplate(args[1], player.PlayerId);
                    else Utils.SendMessage($"{GetString("ForExample")}:\n{args[0]} test", player.PlayerId);
                    break;
