    ...
    class EndGamePatch
    {
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] ref EndGameResult endGameResult)
        {
            ...
            if (GameManager.Instance.DidImpostorsWin(endGameResult.GameOverReason))
            {
                ...
                {
                    //if (p.GetCustomSubRole() == CustomRoles.LoversRecode) continue;
                    bool canWin = p.Is(RoleType.Impostor) || p.Is(RoleType.Madmate) || p.Is(CustomRoles.CrewPostor) || p.Is(CustomRoles.Satan) || p.Is(CustomRoles.CPSchrodingerCat) || p.Is(CustomRoles.CorruptedSheriff);
                    if (canWin) winner.Add(p);
            ...
            if (Main.currentWinner == CustomWinner.Dracula)
            {
                ...
                foreach (var p in PlayerControl.AllPlayerControls)
                {
                    if (p.Is(CustomRoles.Dracula)) winner.Add(p);
                }
            }
            ...
            foreach (var pc in PlayerControl.AllPlayerControls)
            {
                ...
                //sellout maybe
                if (pc.Is(CustomRoles.Sellout) && !pc.Data.IsDead && Main.currentWinner != CustomWinner.Draw && Main.currentWinner != CustomWinner.Terrorist && Main.currentWinner != CustomWinner.Child && Main.currentWinner != CustomWinner.Jester && Main.currentWinner != CustomWinner.Executioner && Main.currentWinner != CustomWinner.Swapper)
                {
                    winner.Add(pc);
                    Main.additionalwinners.Add(AdditionalWinners.sellout);
                }
                //chancer
                if (pc.Is(CustomRoles.Chancer) && !pc.Data.IsDead && Main.currentWinner != CustomWinner.Draw && Main.currentWinner != CustomWinner.Terrorist && Main.currentWinner != CustomWinner.Child && Main.currentWinner != CustomWinner.Jester && Main.currentWinner != CustomWinner.Executioner && Main.currentWinner != CustomWinner.Swapper)
                {
                    winner.Add(pc);
                    Main.additionalwinners.Add(AdditionalWinners.chancer);
                }
            ...
            foreach (var pc in PlayerControl.AllPlayerControls)
            {
                ...
                if (pc.Is(CustomRoles.Lawyer))
                {
                    ...
                    {
                        if (winnerIDs.Contains(client.Value))
                        {
                            winner.Add(pc);
