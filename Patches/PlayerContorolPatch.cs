...
namespace TownOfHost
{
        ...
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            if (killer.PlayerId != target.PlayerId)
            {
                switch (killer.GetCustomRole())
                {
                    ...
                    case CustomRoles.Reviver:
                    case CustomRoles.Opportunist:
                        return false;
                    case CustomRoles.Sellout:
                    case CustomRoles.Chancer:
                    case CustomRoles.Satan:
                        return true;
                    default:
                        break;
                }
                        ...
                        else
                        {
                            ...
                            switch (killer.GetCustomRole())
                            {
                                ...
                                case CustomRoles.Magician:
                                    target.RpcSetCustomRole(CustomRoles.MGSchrodingerCat);
                                    break;
                                case CustomRoles.Satan:
                                    target.RpcSetCustomRole(CustomRoles.RBSchrodingerCat);
                                    break;
                            }
                        ...
                        foreach (var key in Main.CurrentTarget)
                        {
                            ...
                            switch (player.GetCustomRole())
                            {
                                case CustomRoles.Bodyguard:
                                    ...
                                    {
                                        PlayerState.SetDeathReason(player.PlayerId, PlayerState.DeathReason.Bodyguarded); //change to Body
                                    }
                                    else
                                    {
                                ...
                                case CustomRoles.Medic:
                                    //Allow PlagueBearer and Arsonist to do their thing to protected playears
                ...
                switch (killer.GetCustomRole())
                {
                    ...
                    case CustomRoles.Swooper:
                        ...
                        if (Main.IsInvis)
                        {
                            ...
                            if (target.Is(CustomRoles.Kamikaze) && !killer.Is(CustomRoles.Kamikaze))
                                killer.KamiKilled(target);
                            return false;
                        }
                        break;
                    case CustomRoles.PUMPkinsPotion:
                        if (Main.IsInvispumpkin)
                        {
                            killer.RpcGuardAndKill(target);
                            target.RpcMurderPlayer(target, true);
                            Main.whoKilledWho.Remove(target.Data.PlayerId);
                    ...
                    case CustomRoles.Dracula:
                        ...
                        { //キルキャンセル&自爆処理
                          ...
                          // {
                            Utils.CustomSyncAllSettings();
                            Main.AllPlayerKillCooldown[killer.PlayerId] = Options.draculaCooldown.GetFloat();
                            killer.CustomSyncSettings(); //負荷軽減のため、killerだけがCustomSyncSettingsを実行
                            killer.RpcGuardAndKill(target);
                            Main.BitPlayers.Add(target.PlayerId, (killer.PlayerId, 0f));
                            return false;
                        ...
                        else
                        {
                            ...
                            if (Options.draculaBuff.GetBool()) //Poisoner Buff will still make Poisoner report but later.
                            {
                                Utils.CustomSyncAllSettings();
                                Main.AllPlayerKillCooldown[killer.PlayerId] = Options.draculaCooldown.GetFloat();
                                killer.CustomSyncSettings(); //負荷軽減のため、killerだけがCustomSyncSettingsを実行
    ...
    class MurderPlayerPatch
    {
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            if (Main.CurrentTarget.ContainsKey(target.PlayerId) && target.Is(CustomRoles.Paramedic))
            {
                Main.CurrentTarget.Remove(target.PlayerId);
            }
                    ...
                    if (target.PlayerId == gaTarget.Value && !ga.Data.IsDead)
                    {
                        ...
                        else
                        {
                            ...
                            if (Options.CRoleGuardianAngelChangeRoles[Options.WhenGaTargetDies.GetSelection()] != CustomRoles.Reviver)
                                ga.RpcSetCustomRole(Options.CRoleGuardianAngelChangeRoles[Options.WhenGaTargetDies.GetSelection()]);
                            else
            ...
            {
                List<byte> RemoveLWKey = new();
                foreach (var lwTarget in Main.LawyerTarget)
                {
                    var lw = Utils.GetPlayerById(lwTarget.Key);
                    if (lw == null) continue;
                    if (target.PlayerId == lwTarget.Value && !lw.Data.IsDead)
                    {
                        lw.RpcSetCustomRole(Options.CRoleLawyerChangeRoles[Options.WhenLawyerTargetDies.GetSelection()]);
                        RemoveLWKey.Add(lwTarget.Key);
                    }
                }
                foreach (var RemoveKey in RemoveLWKey)
                {
                    Main.LawyerTarget.Remove(RemoveKey);
                    RPC.RemoveLWKey(RemoveKey);
                }
            }
    ...
    class ShapeshiftPatch
    {
        public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
        {
            ...
            if (Main.CanUseShapeshiftAbilites)
            {
                ...
                if (shapeshifter.Is(CustomRoles.Satan) && !shapeshifter.Data.IsDead && Main.satanCanSuicide &&
                    shapeshifting)
                {
                    shapeshifter.RpcMurderPlayer(shapeshifter, true);
    ...
    class ReportDeadBodyPatch
    {
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] GameData.PlayerInfo target)
        {
            ...
            if (AgiTater.IsEnable() && AmongUsClient.Instance.AmHost)
            {
                if (AgiTater.CurrentBombedPlayer != 255 && AgiTater.ReportBait.GetBool())
                {
                    ...
                    if (bombed.GetCustomSubRole() is CustomRoles.Bait)
                    {
                        ...
                        PlayerState.SetDeathReason(bombed.PlayerId, PlayerState.DeathReason.tatered);
           ...
           /* role is CustomRoles.Impostor or
                    ...
                    try
                    {
                        if (Main.whoKilledWho.ContainsKey(target.PlayerId))
                        {
                            ...
                            else
                            {
                                ...
                                else if (PlayerState.GetDeathReason(target.PlayerId) == PlayerState.DeathReason.Kamikazed)
                                    Utils.SendMessage("Kamikaze took you with them.", __instance.PlayerId);
                                ...
                                else if (PlayerState.GetDeathReason(target.PlayerId) == PlayerState.DeathReason.Drained)
                                    Utils.SendMessage("The body had its blood Drained by Dracula.", __instance.PlayerId);
                                else if (PlayerState.GetDeathReason(target.PlayerId) == PlayerState.DeathReason.Hustled)
                                    Utils.SendMessage("Had There Vote Hustled.", __instance.PlayerId);
                ...
                if (__instance.Is(CustomRoles.Amnesiac))
                {
                    ...
                    switch (target.GetCustomRole().GetRoleType())
                    {
                        ...
                        case RoleType.Neutral:
...

                            {
                                ...
                                else
                                {
                                    ...
                                    __instance.RpcSetCustomRole(CustomRoles.Sellout);
                                    __instance.RpcSetCustomRole(CustomRoles.Chancer);
                            ...
                            else
                            {
                                switch (target.GetCustomRole())
                                {
                                    ...
                                    default:
                                        ...
                                        __instance.RpcSetCustomRole(CustomRoles.Sellout);
                                        __instance.RpcSetCustomRole(CustomRoles.Chancer);
                                        __instance.RpcSetCustomRole(CustomRoles.SerialKiller);
                                        break;
                                }
                            }
                        ...
                        case RoleType.Impostor:
                            ...
                            if (reported.IsImpostor())

                            {
                                if (!reported.Is(CustomRoles.Assassin) && !reported.Is(CustomRoles.Swooper) && !reported.Is(CustomRoles.Vampire) && !reported.Is(CustomRoles.Pickpocket))
                                {
                                    __instance.RpcSetCustomRole(reported.GetCustomRole());
                                    switch (target.GetCustomRole())
                                    {
            ...
            foreach (var bp in Main.BitPlayers)
            {
                ...
                if (!bitten.Is(CustomRoles.Veteran))
                {
                    if (!bitten.Data.IsDead)
                    {
                        ...
                        if (bitten.Is(CustomRoles.Kamikaze))
                            Main.KilledKami.Add(vampireID);
            ...
            foreach (var killer in Main.KilledKami)
            {
                var realKiller = Utils.GetPlayerById(killer);
                if (!realKiller.Is(CustomRoles.Pestilence))
                {
                    if (!realKiller.inVent)
                    {
                        ...
                        if (realKiller.Is(CustomRoles.Survivor))
                        {
                            ...
                            realKiller.RpcMurderPlayer(realKiller, true);
                        PlayerState.SetDeathReason(killer, PlayerState.DeathReason.Kamikazed);
                        PlayerState.SetDead(killer);
                    }
                }
               // Main.KilledDemo.Remove(killer);
                Main.KilledKami.Remove(killer);
                MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.NotifyKamiKill, Hazel.SendOption.Reliable, -1);
                writer.Write(true);
                writer.Write(killer);
                AmongUsClient.Instance.FinishRpcImmediately(writer);
            }
        ...
        public static void Postfix(PlayerControl __instance)
        {
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (GameStates.IsInTask && CustomRoles.Vampire.IsEnable())
                {
                    ...
                    if (GameStates.IsInTask && CustomRoles.Dracula.IsEnable())
                    {
                        if (Main.BitPlayers.ContainsKey(player.PlayerId))
                        {
                            //__instance:キルされる予定のプレイヤー
                            ...
                            byte vampireID = Main.BitPlayers[player.PlayerId].Item1;
                            float killTimer = Main.BitPlayers[player.PlayerId].Item2;
                            if (killTimer >= Options.draculaDelay.GetFloat())
                            {
                                var bitten = player;
                                if (!bitten.Data.IsDead)
                                {
                                    PlayerState.SetDeathReason(bitten.PlayerId, PlayerState.DeathReason.Screamed);
                                    var vampirePC = Utils.GetPlayerById(vampireID);
                                    if (!bitten.Is(CustomRoles.Bait))
                                    {
                                        if (vampirePC.IsAlive())
                                        ...
                                        else
                                        {
                                            ...
                                            if (Options.draculaBuff.GetBool())
                                            {
                                                if (bitten.Is(CustomRoles.Survivor)) { Utils.CheckSurvivorVest(bitten, vampirePC); }
                                                else
                                                    bitten.RpcMurderPlayer(bitten, true);
            ...
            {
                 if (GameStates.IsInTask && !GameStates.IsLobby && (__instance.Is(CustomRoles.Sheriff) || __instance.Is(CustomRoles.Hustler) || __instance.Is(CustomRoles.Dracula) || __instance.Is(CustomRoles.PoisonMaster) || __instance.Is(CustomRoles.AgiTater) || __instance.Is(CustomRoles.NeutWitch) || __instance.Is(CustomRoles.Investigator) || __instance.Is(CustomRoles.Escort) || __instance.Is(CustomRoles.Crusader) || __instance.Is(CustomRoles.Hitman) || __instance.Is(CustomRoles.Janitor) || __instance.Is(CustomRoles.Painter) || __instance.Is(CustomRoles.Marksman) || __instance.Is(CustomRoles.BloodKnight) || __instance.Is(CustomRoles.Sidekick) || __instance.Is(CustomRoles.CorruptedSheriff) || __instance.GetRoleType() == RoleType.Coven || __instance.Is(CustomRoles.Arsonist) || __instance.Is(CustomRoles.Werewolf) || __instance.Is(CustomRoles.TheGlitch) || __instance.Is(CustomRoles.Juggernaut) || __instance.Is(CustomRoles.PlagueBearer) || __instance.Is(CustomRoles.Pestilence) || __instance.Is(CustomRoles.Jackal)) && !__instance.Data.IsDead)
                {
            ...
            if (RoleText != null && __instance != null)
            {
                ...
                if (GameStates.IsInGame)
                {
                    ...
                    foreach (var client in Main.LawyerTarget)
                    {
                        if ((seer.PlayerId == client.Key || seer.Data.IsDead) && //seerがKey or Dead
                        target.PlayerId == client.Value) //targetがValue
                            Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Lawyer)}>✓</color>";
                    }
                    ...
                    {
                        if (!target.Data.Disconnected)
                            Mark += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Medic), Utils.GetVitalText(target.PlayerId))})";
                    }
                    ...
                    //ハートマークを付ける(会議中MOD視点)  show modifier name try 2 
                    ...
                    //show modifier this is what makes it work for host only
                    if (__instance.GetCustomSubRole() == CustomRoles.Bait && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Bait)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Bait)}> Bait </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Torch && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Torch)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Torch)}> Torch </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Bewilder && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Bewilder)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Bewilder)}> Bewilder </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Diseased && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Diseased)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Diseased)}> Diseased </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Flash && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Flash)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Flash)}> Flash </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Escalation && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Escalation)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Escalation)}> Escalation </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.TieBreaker && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.TieBreaker)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.TieBreaker)}> TieBreaker </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Oblivious && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Oblivious)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Oblivious)}> Oblivious </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Sleuth && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Sleuth)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Sleuth)}> Sleuth </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Watcher && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Watcher)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Watcher)}> Watcher </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Obvious && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Obvious)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Obvious)}> Obvious</color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.DoubleShot && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.DoubleShot)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.DoubleShot)}> DoubleShot </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Mayor && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Mayor)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Mayor)}> Mayor </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Demolitionist && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Demolitionist)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Demolitionist)}> Demolitionist </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Transporter && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Transporter)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Transporter)}> Transporter </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Doctor && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Doctor)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Doctor)}> Doctor </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Trapper && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Trapper)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Trapper)}> Trapper </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Veteran && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Veteran)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Veteran)}> Veteran </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.PUMPkinsPotion && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.PUMPkinsPotion)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.PUMPkinsPotion)}> PUMPkinsPotion </color>";
                    }
                    if (__instance.GetCustomSubRole() == CustomRoles.Bastion && PlayerControl.LocalPlayer.GetCustomSubRole() == CustomRoles.Bastion)
                    {
                        Mark += $"<color={Utils.GetRoleColorCode(CustomRoles.Bastion)}> Bastion </color>";
                    }
                    ...
                    if (GameStates.IsInTask && Options.ReviverArrow.GetBool() && target.Is(CustomRoles.Reviver))
                    {
    ...
    class PlayerStartPatch
    {
        public static void Postfix(PlayerControl __instance)
        {
            ...
            roleText.transform.localPosition = new Vector3(0f, 0.200f, 0f);
            roleText.fontSize = 1.50f;
    ...
    class EnterVentPatch
    {
        public static void Postfix(Vent __instance, [HarmonyArgument(0)] PlayerControl pc)
        {
            if (AmongUsClient.Instance.AmHost)
            {
                    ...
                    if (doused.Count != 0 && doused.Count >= Options.MinDouseToInginite.GetInt() | skip)
                    {
                        foreach (var pcd in doused)
                        {
                            ...
                            if (pcd.Is(CustomRoles.Veteran) && Main.VetIsAlerted)
                            {
                                ...
                                PlayerState.SetDeathReason(pc.PlayerId, PlayerState.DeathReason.vetted);//change to vetted
                ...
                if (pc.Is(CustomRoles.Wizard))
                {
                    if (!Main.IsInviswizard && Main.CanGoInviswizard)
                        skipCheck = true;
                    if (Main.IsInviswizard)
                    {
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                        ...
                        Options.WizardCooldown.GetFloat(), "WizardCooldown", true);
                    }
                }

                if (pc.Is(CustomRoles.PUMPkinsPotion))
                {
                    if (!Main.IsInvispumpkin && Main.CanGoInvispumpkin)
                        skipCheck = true;
                    if (Main.IsInvispumpkin)
                    {
                        pc.MyPhysics.RpcBootFromVent(__instance.Id);
                        ...
                        Options.PUMPkinsPotionCooldown.GetFloat(), "PUMPkinsPotionCooldown", true);
                    }
                }
    ...
    class CoEnterVentPatch
    {
        public static bool Prefix(PlayerPhysics __instance, [HarmonyArgument(0)] int id)
        {
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                __instance.myPlayer.Is(CustomRoles.Sellout) ||
                __instance.myPlayer.Is(CustomRoles.Satan) ||
                __instance.myPlayer.Is(CustomRoles.Chancer) ||
                __instance.myPlayer.Is(CustomRoles.NeutWitch) ||
                __instance.myPlayer.Is(CustomRoles.Amnesiac) ||
                __instance.myPlayer.Is(CustomRoles.Reviver) ||
                ...
                if (__instance.myPlayer.Is(CustomRoles.Wizard))
                {
                    new LateTask(() =>
                    {
                        if (!Main.IsInviswizard && Main.CanGoInviswizard)
                        {
                            ...
                            Options.WizardDuration.GetFloat(), "WizardDuration", true);
                        }
                        else
                        {
                            __instance.myPlayer.MyPhysics.RpcBootFromVent(id);
                ...
                if (__instance.myPlayer.Is(CustomRoles.PUMPkinsPotion))
                {
                    new LateTask(() =>
                    {
                        if (!Main.IsInvispumpkin && Main.CanGoInvispumpkin)
                        {
                            ...
                            Options.PUMPkinsPotionDuration.GetFloat(), "PUMPkinsPotionDuration", true);
                        }
                        else
                        {
                            __instance.myPlayer.MyPhysics.RpcBootFromVent(id);
