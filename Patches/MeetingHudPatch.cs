...
namespace TownOfHost
{
    ...
    class CheckForEndVotingPatch
    {
        public static bool Prefix(MeetingHud __instance)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < __instance.playerStates.Length; i++)
                {
                    ...
                    if (voter.GetCustomRole() is CustomRoles.Oracle or CustomRoles.Bodyguard or CustomRoles.Medic or CustomRoles.Paramedic)
                    {
                    ...
                    if (IsHustlerMayor(ps.TargetPlayerId))
                    {
                        for (var i3 = 0; i3 < Main.MayorUsedButtonCount[ps.TargetPlayerId]; i3++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
                   ...
                   /* if (IsSheriffMayor(ps.TargetPlayerId))
                    {
                        for (var i3 = 0; i3 < Main.MayorUsedButtonCount[ps.TargetPlayerId]; i3++)
                        {
                            statesList.Add(new MeetingHud.VoterState()
        ...
        public static bool IsHustlerMayor(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Hustler);
        }
       /* public static bool IsSheriffMayor(byte id)
        {
            var player = PlayerControl.AllPlayerControls.ToArray().Where(pc => pc.PlayerId == id).FirstOrDefault();
            return player != null && player.Is(CustomRoles.Sheriff);
        } */
            ...
            foreach (var pva in __instance.playerStates)
            {
                try
                {
                    if (!Options.RolesLikeToU.GetBool())
                    {
                        ...
                        roleTextMeeting.fontSize = 0.9f;  //maybe role size
                    ...
                    foreach (var pc in PlayerControl.AllPlayerControls)
                    {
                        ...
                        switch (role)
                        {
                            ...
                            case CustomRoles.Lawyer:
                                Main.LawyerTarget.TryGetValue(pc.PlayerId, out var lwId);
                                if (!Utils.GetPlayerById(lwId).GetCustomRole().IsCrewmate())
                                    badPlayers.Add(pc);
                                break;
                        }
                        switch (role.GetRoleType())
                        {
                            case RoleType.Crewmate:
                                ...
                                if (role is CustomRoles.Sheriff or CustomRoles.Veteran or CustomRoles.Chancer
                                    or CustomRoles.Bodyguard or CustomRoles.Sellout or CustomRoles.Crusader or CustomRoles.Reviver or CustomRoles.TimeManager or CustomRoles.Child or CustomRoles.Bastion or CustomRoles.Demolitionist or CustomRoles.Kamikaze or CustomRoles.NiceGuesser) badPlayers.Add(pc);
                                break;
                ...
                _ = new LateTask(() =>
                {
                    ...
                    _ = new LateTask(() =>
                    {
                        ...
                        if (!Main.FirstMeetingPassed)
                        {
                            ...
                            {
                                var role = pc.GetCustomRole();
                                var subrole = pc.GetCustomSubRole();

                                if (role.IsVanilla()) Utils.SendMessage($"Vanilla roles currently have no description.", pc.PlayerId);
                                else Utils.SendMessage(GetString(role.ToString()) + GetString($"{role}InfoLong"), pc.PlayerId);
                                if (subrole != CustomRoles.NoSubRoleAssigned)
                                    Utils.SendMessage(GetString(subrole.ToString()) + GetString($"{subrole}InfoLong"), pc.PlayerId);
                            }
                        ...
                        {
                            Main.FirstMeetingPassed = true;
                            var text = "SUBSCRIBE TO @pumpkingaming5548 for modded games USE this time to check your role by using \"/m\"!";
                    ...
                    switch (seer.GetCustomRole())
                    {
                        ...
                        case CustomRoles.Paramedic:
                            if (target.Data.IsDead)
                                pva.NameText.text += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Paramedic), target.GetDeathReason())})";
                            break;
                        ...
                        //Lawywer Target
                        case CustomRoles.Lawyer:
                            if (Main.LawyerTarget.TryGetValue(seer.PlayerId, out var LWId) && target.PlayerId == LWId)
                            { //target
                                pva.NameText.text += Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Lawyer), "♦");
                                if (Options.LawyerknowsRole.GetBool())
                                    pva.AddRoleText(false);
                            }
                            break;
