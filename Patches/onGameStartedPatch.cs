...
namespace TownOfHost
{
    ...
    class ChangeRoleSettings
    {
        public static void Postfix(AmongUsClient __instance)
        {
            try
            {
                ...
                Main.rolesRevealedNextMeeting1 = new List<byte>();
    ...
    class SelectRolesPatch
    {
        ...
        public static void Postfix()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (RoleGoingInList(CustomRoles.Doctor))
                        GiveModifier(CustomRoles.Doctor);
                    if (RoleGoingInList(CustomRoles.Mayor))
                        GiveModifier(CustomRoles.Mayor);
                    if (RoleGoingInList(CustomRoles.Demolitionist))
                        GiveModifier(CustomRoles.Demolitionist);
                    if (RoleGoingInList(CustomRoles.Trapper))
                        GiveModifier(CustomRoles.Trapper);
                    if (RoleGoingInList(CustomRoles.Mystic))
                        GiveModifier(CustomRoles.Mystic);
                    if (RoleGoingInList(CustomRoles.Transporter))
                        GiveModifier(CustomRoles.Transporter);
                    if (RoleGoingInList(CustomRoles.Obvious))
                        GiveModifier(CustomRoles.Obvious);
                    if (RoleGoingInList(CustomRoles.Veteran))
                        GiveModifier(CustomRoles.Veteran);
                    if (RoleGoingInList(CustomRoles.Escapist))
                        GiveModifier(CustomRoles.Escapist);
                    if (RoleGoingInList(CustomRoles.PUMPkinsPotion))
                        GiveModifier(CustomRoles.PUMPkinsPotion);
                    if (RoleGoingInList(CustomRoles.Bastion))
                        GiveModifier(CustomRoles.Bastion);
                    ...
                    foreach (var pc in PlayerControl.AllPlayerControls)
                    {
                        ...
                        switch (pc.GetCustomRole())
                        {
                            ...
                            case CustomRoles.GuardianAngelTOU:
                                ...
                                break;
                            //Determin lawyer's client
                            case CustomRoles.Lawyer:
                                List<PlayerControl> LWList = new();
                                rand = new System.Random();
                                foreach (var target in PlayerControl.AllPlayerControls)
                                {
                                    if (pc == target) continue;
                                    ...
                                    if (target.Is(CustomRoles.Phantom)) continue;
                                    //Not Crew?
                                    var rt = target.GetRoleType();
                                    if (rt == RoleType.Crewmate) continue;
                                    LWList.Add(target);
                                }
                                var client = LWList[rand.Next(LWList.Count)];
                                Main.LawyerTarget.Add(pc.PlayerId, client.PlayerId);
                                Main.LawyerClient.Add(pc);
                                Main.LawyerClient.Add(client);
                                RPC.SendLWTarget(pc.PlayerId, client.PlayerId);
                                Logger.Info($"{pc.GetNameWithRole()}:{client.GetNameWithRole()}", "Lawyer");
                                break;
        ...
        private static void GiveModifier(CustomRoles role, int RawCount = -1)
        {
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                    ...
                    if (player.Is(CustomRoles.GM)) continue;
                {
                    ...
                    {
                        case CustomRoles.Sleuth:
                            if (player.GetCustomRole() is CustomRoles.Medium or CustomRoles.Amnesiac or CustomRoles.Reviver or CustomRoles.Vulture or CustomRoles.Cleaner) continue;
                            break;
                        ...
                        case CustomRoles.Bastion:
                            if (player.GetCustomRole() is CustomRoles.Wizard or CustomRoles.Swooper or CustomRoles.Paramedic or CustomRoles.Reviver) continue;
                            break;
        ...
        private static void GiveModifier2(CustomRoles role, int RawCount = -1)
        {
            if (!role.IsEnable()) return;
            var allPlayers = new List<PlayerControl>();
            if (role is CustomRoles.Flash)
                    ...
                    if (player.Is(CustomRoles.GM)) continue;
                {
                    switch (role)
                    {
                        ...
                        case CustomRoles.Demolitionist:
                            ...
                            break;
                        case CustomRoles.Bastion:
                            if (player.GetCustomRole() is CustomRoles.Wizard or CustomRoles.Swooper or CustomRoles.Paramedic) continue;
                            break;
                        case CustomRoles.Diseased:
                            if (player.GetCustomRole() is CustomRoles.Kamikaze) continue;
                            break;
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                allPlayers.Remove(player);
                Main.AllPlayerCustomSubRoles[player.PlayerId] = role;
                Logger.Info("役職設定:" + player?.Data?.PlayerName + " = "  + role.ToString(), "Assign Modifier2" + " + " + player.GetCustomRole().ToString());
            }
        }
