...
namespace TownOfHost
{
    static class ExtendedPlayerControl
    {
        ...
        public static void SetDefaultRole(this PlayerControl player)
        {
            switch (player.GetRoleType())
            {
                case RoleType.Crewmate:
                    ...
                    player.RpcSetCustomRole(CustomRoles.Chancer);
                    RoleManager.Instance.SetRole(player, RoleTypes.Crewmate);
                    break;
                case RoleType.Impostor:
                    player.RpcSetCustomRole(CustomRoles.Impostor);
                    player.RpcSetCustomRole(CustomRoles.Satan);
                ...
                case RoleType.Neutral:
                    if (!player.GetCustomRole().IsNeutralKilling())
                    {
                        ...
                        player.RpcSetCustomRole(CustomRoles.Sellout);
                        RoleManager.Instance.SetRole(player, RoleTypes.Crewmate);
                    }
                    break;
                case RoleType.Coven:
        ...
        public static bool hidePlayerName(PlayerControl source, PlayerControl target)
        {
            ...
            if (target.Is(CustomRoles.Wizard) && Main.IsInviswizard) return true;
            if (target.Is(CustomRoles.PUMPkinsPotion) && Main.IsInvispumpkin) return true;
        ...
        public static void CustomSyncSettings(this PlayerControl player)
        {
            ...
            switch (player.GetCustomRole())
            {
                ...
                case CustomRoles.Kamikaze:
                    Kamikaze.ApplyGameOptions(options, player.PlayerId);
                    break;
        ...
        public static bool SameTeamAsTarget(this PlayerControl killer, PlayerControl target)
        {
            ...
            if (killer.Is(CustomRoles.Magician) && target.Is(CustomRoles.MGSchrodingerCat)) return true;
            if (killer.Is(CustomRoles.Satan) && target.Is(CustomRoles.RBSchrodingerCat)) return true;
        ...
        public static void ResetKillCooldown(this PlayerControl player, bool meeting = false)
        {
            ...
            switch (player.GetCustomRole())
            {
                ...
                case CustomRoles.Painter:
                    ...
                    break;
                case CustomRoles.Dracula:
                    Main.AllPlayerKillCooldown[player.PlayerId] = Options.draculaCooldown.GetFloat();
                    break;
                case CustomRoles.Vampire:
                    Main.AllPlayerKillCooldown[player.PlayerId] = Options.vampCooldown.GetFloat();
                    break;
        ...
        public static void KamiKilled(this PlayerControl killer, PlayerControl target)
        {
            Logger.Info($"{killer?.Data?.PlayerName}はTrapperだった", "KilledKami");
            Logger.Info($"{target?.Data?.PlayerName}はTrapperだった", "IsDemo");
            //killer.Data.PlayerName += $"<color={Utils.GetRoleColorCode(CustomRoles.Demolitionist)}>▲</color>";
            Main.KilledKami.Add(killer.PlayerId);
            killer.CustomSyncSettings();
            ...
            new LateTask(() =>
            {
                ...
                if (!GameStates.IsMeeting)
                {
                    Main.KilledKami.Remove(killer.PlayerId);
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.NotifyKamiKill, Hazel.SendOption.Reliable, -1);
                    writer.Write(true);
                    writer.Write(killer.PlayerId);
                    AmongUsClient.Instance.FinishRpcImmediately(writer);
                    ...
                    if (!killer.inVent && !killer.Data.IsDead)
                    {
                        if (!killer.Is(CustomRoles.Pestilence))
                        {
                            ...
                            if (killer.protectedByGuardianThisRound)
                                killer.RpcMurderPlayer(killer, true);
                            killer.RpcMurderPlayer(killer, true);
                            PlayerState.SetDeathReason(killer.PlayerId, PlayerState.DeathReason.Kamikazed);
                            PlayerState.SetDead(killer.PlayerId);
                        }
                    }
                    else
            ...
            }, Kamikaze.KamiSuicideTime.GetFloat(), "Kami Time", true);
        }
        ...
        public static string GetDeathMessage(this PlayerControl pc)
        {
            try
            {
                ...
                if (PlayerState.GetDeathReason(pc.PlayerId) == PlayerState.DeathReason.Kamikazed)
                    return "Kamikaze took you with them.";
                ...
                switch (role)
                {
                    ...
                    case CustomRoles.Magician:
                        return $"They were knocked out by the Magician.";
                    case CustomRoles.Veteran:
