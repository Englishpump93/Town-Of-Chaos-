...
namespace TownOfHost
{
        ...
        public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] byte callId, [HarmonyArgument(1)] MessageReader reader)
        {
            ...
            switch (rpcType)
            {
                ...
                case CustomRPC.RemoveGATarget:
                    ...
                    break;
                //Set and Remove Lawyer Target
                case CustomRPC.SetLWTarget:
                    byte lwId = reader.ReadByte();
                    byte clientIds = reader.ReadByte();
                    Main.GuardianAngelTarget[lwId] = clientIds;
                    break;
                case CustomRPC.RemoveLWTarget:
                    byte clId = reader.ReadByte();
                    Main.GuardianAngelTarget.Remove(clId);
                    break;
                ...
                case CustomRPC.NotifyDemoKill: // DONE
                    ...
                    break;
                case CustomRPC.NotifyKamiKill: // DONE
                    var remove1 = reader.ReadBoolean();
                    if (!remove1)
                        Main.KilledKami.Add(reader.ReadByte());
                    else
                        Main.KilledKami.Remove(reader.ReadByte());
                    break;
                ...
                case CustomRPC.RpcSetHustlerProgress:
                    var killereee = reader.ReadByte();
                    Main.HustlerKills[killereee] = reader.ReadInt32();
                    break;
    ...
    static class RPC
    {
        ...
        //Add Lawyer and Client to variables
        public static void SendLWTarget(byte lwId, byte clientId)
        {
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetLWTarget, Hazel.SendOption.Reliable, -1);
            writer.Write(lwId);
        ...
        //Remove Lawyer Target When Dead
        public static void RemoveLWKey(byte lKey)
        {
            if (!AmongUsClient.Instance.AmHost) return;
            MessageWriter lwriter = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.RemoveLWTarget, Hazel.SendOption.Reliable, -1);
            lwriter.Write(lKey);
            AmongUsClient.Instance.FinishRpcImmediately(lwriter);
        }
