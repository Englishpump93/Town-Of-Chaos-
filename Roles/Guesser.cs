...
namespace TownOfHost
{
    public static class Guesser
    {
        ...
        public static CustomOption AssCanGuessNonNK;
        public static CustomOption AssCanseekami;
        ...
        public static CustomOption VigiCanGuessModifers;
        public static CustomOption VigiCanGuessNonNK;
        ...
        public static CustomOption PirateCanGuessNonNK;
        public static CustomOption PirateCanGuessKami;
        ...
        {
            Options.SetupRoleOptions(Id + 100, CustomRoles.Assassin, AmongUsExtensions.OptionType.Impostor);
            AssKillCount = CustomOption.Create(Id + 110, Color.white, "GuesserShootLimit", AmongUsExtensions.OptionType.Impostor, 1, 1, 15, 1, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanKillMultiplePerMeeting = CustomOption.Create(Id + 111, Color.white, "CanKillMultiplePerMeeting", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessCrewmate = CustomOption.Create(Id + 112, Color.white, "CanGuessCrewmate", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessNeutralBenign = CustomOption.Create(Id + 113, Color.white, "CanGuessNeutralBenign", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessNeutralEvil = CustomOption.Create(Id + 114, Color.white, "CanGuessNeutralEvil", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessNeutralKilling = CustomOption.Create(Id + 115, Color.white, "CanGuessNeutralKilling", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessCrewModifers = CustomOption.Create(Id + 116, Color.white, "CanGuessCrewModifers", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessNonNK = CustomOption.Create(Id + 116, Color.white, "CanGuessNonNK", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
           // AssCanseekami = CustomOption.Create(Id + 116, Color.white, "CanGuessKamikaze", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessLovers = CustomOption.Create(Id + 117, Color.white, "CanGuessLovers", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssCanGuessAfterVoting = CustomOption.Create(Id + 118, Color.white, "CanGuessAfterVoting", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            AssHideCommand = CustomOption.Create(Id + 119, Color.white, "HideCommand", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Assassin]);
            ...
            VigiCanGuessModifers = CustomOption.Create(Id + 316, Color.white, "CanGuessCrewModifers", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.NiceGuesser]);
            VigiCanGuessNonNK = CustomOption.Create(Id + 316, Color.white, "CanGuessNonNK", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.NiceGuesser]);
            ...
            PirateCanGuessNonNK = CustomOption.Create(Id + 316, Color.white, "CanGuessNonNK", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Pirate]);
           // PirateCanGuessKami = CustomOption.Create(Id + 316, Color.white, "CanGuessKamikaze", AmongUsExtensions.OptionType.Impostor, false, Options.CustomRoleSpawnChances[CustomRoles.Pirate]);
                ...
                foreach (var target in PlayerControl.AllPlayerControls)
                {
                    if (playerId == $"{target.PlayerId}" && GuesserShootLimit[killer.PlayerId] != 0)//targetnameが人の名前で弾数が０じゃないなら続行
                    {
                        ...
                        if (target.GetCustomRole() == r | target.GetCustomSubRole() == r)//当たっていた場合
                        {
                            ...
                            if ((target.GetCustomRole() == CustomRoles.Crewmate && !CanShootRegularCrewmate(killer.GetCustomRole())) || (target.GetCustomRole() == CustomRoles.Egoist && killer.Is(CustomRoles.Assassin))) return;
                ...
                case CustomRoles.Assassin:
                    ...
                    switch (roleTypeAss)
                    {
                        ...
                        case NeutralRoleType.None:
                            if (roleToAgree.Crewguessed()) return AssCanGuessCrewmate.GetBool();
                            if (roleToAgree.IsCrewmate()) return true;
                            if (roleToAgree.IsModifier()) return AssCanGuessCrewModifers.GetBool();
                            if (roleToAgree.IsNonNk()) return AssCanGuessNonNK.GetBool();
                           // if (roleToAgree.Canbeseen()) return AssCanseekami.GetBool();
                ...
                case CustomRoles.NiceGuesser:
                    ...
                    switch (roleTypeVigi)
                    {
                        ...
                        case NeutralRoleType.None:
                            ...
                            if (roleToAgree.IsModifier()) return VigiCanGuessModifers.GetBool();
                            if (roleToAgree.IsNonNk()) return VigiCanGuessNonNK.GetBool();
                            break;
                    }
                    break;
                case CustomRoles.Pirate:
                    ...
                    switch (roleType)
                    {
                        ...
                        case NeutralRoleType.None:
                            if (roleToAgree.Crewguessed()) return PirateCanGuessCrewmate.GetBool();
                            if (roleToAgree.IsCrewmate()) return true;
                            if (roleToAgree.IsImpostorTeam()) return PirateCanGuessImpostorRoles.GetBool();
                            if (roleToAgree.IsModifier()) return PirateCanGuessCrewModifers.GetBool();
                            if (roleToAgree.IsNonNk()) return PirateCanGuessNonNK.GetBool();
                           // if (roleToAgree.Canbeseen()) return PirateCanGuessKami.GetBool();
        ...
        public static void SetRoleAndNumber()//役職を番号で管理
        {
            ...
            foreach (var id in Main.rolesRevealedNextMeeting1)
            {
                revealed.Add(Utils.GetPlayerById(id).GetCustomRole());
            }
            foreach (CustomRoles role in Enum.GetValues(typeof(CustomRoles)))
            {
                ...
                if (role.IsModifier())
                {
                    if (!role.IsModifier() && role != CustomRoles.LoversRecode) continue;
                }
                if (!role.IsImpostorTeam() && role != CustomRoles.Egoist && RoleAgrees(role, CustomRoles.Assassin)) assassinList.Add(role);
